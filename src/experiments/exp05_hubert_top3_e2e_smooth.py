...
# src/experiments/exp05_hubert_top3_e2e_smooth.py
...
def smoothness_loss_second_diff(x: torch.Tensor) -> torch.Tensor:
    """
    Penalize second finite difference to reduce high-frequency jitter.
    x: (B, T)
    """
    if x.shape[1] < 3:
...
def train_exp05_e2e_smooth(
    ...
    End-to-end fine-tuning:
      wav -> HuBERT -> resample 30fps -> Projector -> energy
      loss = MSE(z(energy), z(onset)) + lambda_smooth * smoothness(second_diff(z(energy)))
    ...
    print(f"[exp05] HuBERT trainable: {trainable/1e6:.2f}M / {total/1e6:.2f}M (top {top_k_layers})")
    ...
    for ep in range(epochs):
        print(f"[exp05] Epoch {ep+1}/{epochs}")
        ...
        for idx in order:
            ...
            try:
                ...
                r_30_np = onset_strength_30fps(y, sr, target_fps=30)  # (T,)
                ...
                for sidx in starts:
                    ...
                    with torch.cuda.amp.autocast(enabled=(use_amp and device.startswith("cuda"))):
                        ...
                        # Z-score per-chunk energy
                        energy_z = (energy - energy.mean()) / (energy.std() + 1e-6)

                        loss_rhythm = loss_fn(energy_z, rr)
                        loss_smooth = smoothness_loss_second_diff(energy_z.unsqueeze(0))  # (1,T)
                        loss = loss_rhythm + lambda_smooth * loss_smooth
                ...
                if (step % 10) == 0:
                    print(
                        f"[exp05] step {step:5d} | {wav_path.stem} | "
                        f"loss {track_loss.item():.4f} | chunks {chunks_used}"
                    )
                step += 1

            except Exception as e:
                print(f"[exp05] Error {wav_path.name}: {e}")

        print(f"[exp05] End epoch {ep+1}/{epochs}")
    ...
    print(f"[exp05] Saved projector to {out_projector_ckpt}")
    print(f"[exp05] Saved hubert    to {out_hubert_ckpt}")
